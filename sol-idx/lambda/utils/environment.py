...
def init_logger():
    ...
    if os.getenv("LOCAL_LOG_FORMAT") is None:
        local_log_format = "%(asctime)-26s [%(levelname)-5s] %(name)-25s %(pathname)s:%(lineno)s %(message)s"
    else:
    ...
    if os.getenv("AWS_EXECUTION_ENV") is None:
        ...
        logging.basicConfig(stream=sys.stdout, format=new_format, level=log_level)
    else:
        # NOTE: AWS has already mutated the logger, so we might want to adjust it.
        new_format = aws_log_format
        logging.basicConfig(format=aws_log_format, level=log_level)
